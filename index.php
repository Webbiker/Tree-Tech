<!doctype html>
<html class="no-js" lang="">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="x-ua-compatible" content="ie=edge">
        <title></title>
        <meta name="description" content="">
        <meta name="viewport" content="width=device-width, initial-scale=1">

        <link rel="apple-touch-icon" href="apple-touch-icon.png">
        <!-- Place favicon.ico in the root directory -->

        <link rel="stylesheet" href="/assets/css/screen.css">
        <script src="/assets/js/libs/modernizr-2.8.3.min.js"></script>
    </head>
    <body>
        <!--[if lt IE 8]>
            <p class="browserupgrade">You are using an <strong>outdated</strong> browser. Please <a href="http://browsehappy.com/">upgrade your browser</a> to improve your experience.</p>
        <![endif]-->

        <!-- Add your site or application content here -->
        <div id="page">
            <div class="grid__sizer"></div>
            <header class="grid__item header">
                <a class="header__logo" href="/"><svg><use xlink:href="/assets/svg/logo-treetech.svg#logo"></use></svg></a>

                <div class="header__name">
                    <a href="tel:+31620403332" class="header__call"><svg><use xlink:href="/assets/svg/icons.svg#phone"></use></svg>+31 6 20 40 33 32</a>
                    Bram van den Boom
                </div>
                <div class="header__avatar" style="background-image:url(/assets/img/avatar-klein.png);"></div>
            </header>
            <article class="grid__item grid__about">
                <p>Morem ipsum dolor sit amet, consectetur adipiscing elit. Etiam sollicitudin sollicitudin enim, in luctus turpis consequat ut. Mauris aliquam libero ac pulvinar eleifend. Vestibulum eget rutrum purus. In sed facilisis dolor. Donec scelerisque, lacus nec rutrum ultrices, nulla arcu molestie ipsum, sit amet tempus massa dui a enim.</p>
                <p>Duis semper luctus justo, a convallis sem. Duis semper efficitur ipsum, vel efficitur massa congue in. In eu rhoncus urna, nec consectetur sem. Aliquam erat volutpat. Nam eget mi sed risus rhoncus venenatis nec sed libero. Fusce lectus erat, condimentum eget neque semper, dapibus tincidunt sapien. Curabitur venenatis gravida fermentum. Nullam laoreet lectus et lorem elementum, ac scelerisque magna interdum. Aenean a dictum ligula.</p>
                <p>Nulla sapien lorem, viverra eget mi vel, maximus sodales ex. Quisque viverra feugiat metus, sed placerat enim rutrum vulputate. Phasellus imperdiet enim sed metus finibus, ut pretium nulla cursus. Donec consectetur dignissim purus, vel maximus nunc pulvinar eu. Nunc tellus neque, vulputate sit amet interdum sit amet, pellentesque a orci. Duis aliquam lacinia augue sed aliquam. Ut vulputate tortor a sodales sagittis.</p>
                <p class="text-center"><a href="javascript:;" class="button button--primary" data-modal="contact">Neem contact op <svg><use xlink:href="/assets/svg/icons.svg#envelope"></use></svg></a></p>
            </article>
            <section class="grid__item avatar">
                <img src="/assets/img/avatar-groot.png" alt="Bram van den Boom">
            </section>
            <section class="grid__item grid__gallery">
                <a class="gallery" href="javascript:;" data-modal="bijzettafel">
                    <span class="gallery__title">Bijzettafel</span>
                    <svg class="gallery__zoom"><use xlink:href="/assets/svg/icons.svg#search"></use></svg>                
                    <img class="gallery__img" src="/media/portfolio/treetech-8.jpg">
                </a>
            </section>
            <section class="grid__item grid__gallery">
                <a class="gallery" href="javascript:;" data-modal="lavalamp">
                    <span class="gallery__title">Lavalamp</span>
                    <svg class="gallery__zoom"><use xlink:href="/assets/svg/icons.svg#search"></use></svg>                
                    <img class="gallery__img" src="/media/portfolio/treetech-2.jpg">
                </a>
            </section>
            <section class="grid__item grid__gallery">
                <a class="gallery" href="javascript:;" data-modal="animalart">
                    <span class="gallery__title">Animal art</span>
                    <svg class="gallery__zoom"><use xlink:href="/assets/svg/icons.svg#search"></use></svg>                
                    <img class="gallery__img" src="/media/portfolio/treetech-6.jpg">
                </a>
            </section>
            <section class="grid__item grid__gallery">
                <a class="gallery" href="javascript:;" data-modal="bbqkar">
                    <span class="gallery__title">BBQ kar</span>
                    <svg class="gallery__zoom"><use xlink:href="/assets/svg/icons.svg#search"></use></svg>                
                    <img class="gallery__img" src="/media/portfolio/treetech-9.jpg">
                </a>
            </section>
            <section class="grid__item grid__gallery">
                <a class="gallery" href="javascript:;" data-modal="borrelboom">
                    <span class="gallery__title">Borrelboom</span>
                    <svg class="gallery__zoom"><use xlink:href="/assets/svg/icons.svg#search"></use></svg>                
                    <img class="gallery__img" src="/media/portfolio/treetech-3.jpg">
                </a>
            </section>
            <section class="grid__item grid__gallery">
                <a class="gallery" href="javascript:;" data-modal="hangtafel">
                    <span class="gallery__title">Hangtafel</span>
                    <svg class="gallery__zoom"><use xlink:href="/assets/svg/icons.svg#search"></use></svg>                
                    <img class="gallery__img" src="/media/portfolio/treetech-1.jpg">
                </a>
            </section>
        </div>
        <footer class="footer">
            <p>&copy;Tree-Tech 2016</p>
            <a href="javascript:;" class="button button--primary button--rounded button--up" id="backtotop"><svg><use xlink:href="/assets/svg/icons.svg#caret-up"></use></svg></a>
            <ul class="footer__social">
                <li><a href="javascript:;" class="button button--secondary button--rounded"><svg><use xlink:href="/assets/svg/icons.svg#phone"></use></svg></a></li>
                <li><a href="javascript:;" class="button button--secondary button--rounded"><svg><use xlink:href="/assets/svg/icons.svg#envelope"></use></svg></a></li>
                <li><a href="javascript:;" class="button button--secondary button--rounded"><svg><use xlink:href="/assets/svg/icons.svg#facebook"></use></svg></a></li>
                <li><a href="javascript:;" class="button button--secondary button--rounded"><svg><use xlink:href="/assets/svg/icons.svg#linkedin"></use></svg></a></li>
            </ul>
        </footer>


        <div class="modal" id="bijzettafel">
            <div class="modal__container">
                <div class="modal__wrapper">
                    <div class="modal__img">
                        <p class="modal__pin">                                    
                            <a  data-pin-do="buttonPin" 
                                href="https://www.pinterest.com/pin/create/button/
                                    ?url=http://www.tech-tree.nl
                                    &media=http://localhost:3000/media/portfolio/treetech-8.jpg
                                    &description=Bijzettafel van Tech-Tree.nl" 
                                data-pin-height="28"
                                class="right"></a>
                        </p>
                        <img class="" src="/media/portfolio/treetech-8.jpg">
                    </div>
                    <div class="modal__info">
                        <div class="modal__content"> 
                            <h1 class="modal__title">
                                Bijzettafel
                                <a href="javascript:;" class="modal__close"><svg><use xlink:href="/assets/svg/icons.svg#times-circle"></use></svg></a>
                            </h1>
                            <div class="modal__body">
                                <p>Vivamus condimentum accumsan purus, at blandit felis laoreet eu. Nullam nec leo eget nisi scelerisque convallis.</p>
                                <p>Nunc mollis semper diam auctor ultricies. Suspendisse gravida tempus quam, sit amet rhoncus magna vulputate id. Integer sed libero et enim porta bibendum. Duis rhoncus volutpat pellentesque.</p>
                                <p>Morbi efficitur arcu sit amet felis aliquam eleifend. Vivamus ac pretium sem. Proin posuere venenatis lobortis.</p>
                                <p><strong>Prijs op aanvraag</strong></p>
                            </div>
                            <div class="modal__footer">
                                <p>
                                    <a href="javascript:;" data-modal="contact" class="button button--primary button--small">Interesse in dit product?<svg><use xlink:href="/assets/svg/icons.svg#envelope"></use></svg></a></p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div id="overlay" class="modal__overlay"></div>
        </div>

        <div class="modal" id="lavalamp">
            <div class="modal__container">
                <div class="modal__wrapper">
                    <div class="modal__img">
                        <p class="modal__pin">                                    
                            <a  data-pin-do="buttonPin" 
                                href="https://www.pinterest.com/pin/create/button/
                                    ?url=http://www.tech-tree.nl
                                    &media=http://localhost:3000/media/portfolio/treetech-2.jpg
                                    &description=Lavalamp van Tech-Tree.nl" 
                                data-pin-height="28"
                                class="right"></a>
                        </p>
                        <img class="" src="/media/portfolio/treetech-2.jpg">
                    </div>
                    <div class="modal__info">
                        <div class="modal__content"> 
                            <h1 class="modal__title">
                                Lavalamp
                                <a href="javascript:;" data-modal="contact" class="modal__close"><svg><use xlink:href="/assets/svg/icons.svg#times-circle"></use></svg></a>
                            </h1>
                            <div class="modal__body">
                                <p>Vivamus condimentum accumsan purus, at blandit felis laoreet eu. Nullam nec leo eget nisi scelerisque convallis.</p>
                                <p>Nunc mollis semper diam auctor ultricies. Suspendisse gravida tempus quam, sit amet rhoncus magna vulputate id. Integer sed libero et enim porta bibendum. Duis rhoncus volutpat pellentesque.</p>
                                <p>Morbi efficitur arcu sit amet felis aliquam eleifend. Vivamus ac pretium sem. Proin posuere venenatis lobortis.</p>
                                <p><strong>Prijs op aanvraag</strong></p>
                            </div>
                            <div class="modal__footer">
                                <p>
                                    <a href="javascript:;" data-modal="contact" class="button button--primary button--small">Interesse in dit product?<svg><use xlink:href="/assets/svg/icons.svg#envelope"></use></svg></a></p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div id="overlay" class="modal__overlay"></div>
        </div>

        <div class="modal" id="animalart">
            <div class="modal__container">
                <div class="modal__wrapper">
                    <div class="modal__img">
                        <p class="modal__pin">                                    
                            <a  data-pin-do="buttonPin" 
                                href="https://www.pinterest.com/pin/create/button/
                                    ?url=http://www.tech-tree.nl
                                    &media=http://localhost:3000/media/portfolio/treetech-6.jpg
                                    &description=Animal Art van Tech-Tree.nl" 
                                data-pin-height="28"
                                class="right"></a>
                        </p>
                        <img class="" src="/media/portfolio/treetech-6.jpg">
                    </div>
                    <div class="modal__info">
                        <div class="modal__content"> 
                            <h1 class="modal__title">
                                Animal Art
                                <a href="javascript:;" class="modal__close"><svg><use xlink:href="/assets/svg/icons.svg#times-circle"></use></svg></a>
                            </h1>
                            <div class="modal__body">
                                <p>Vivamus condimentum accumsan purus, at blandit felis laoreet eu. Nullam nec leo eget nisi scelerisque convallis.</p>
                                <p>Nunc mollis semper diam auctor ultricies. Suspendisse gravida tempus quam, sit amet rhoncus magna vulputate id. Integer sed libero et enim porta bibendum. Duis rhoncus volutpat pellentesque.</p>
                                <p>Morbi efficitur arcu sit amet felis aliquam eleifend. Vivamus ac pretium sem. Proin posuere venenatis lobortis.</p>
                                <p><strong>Prijs op aanvraag</strong></p>
                            </div>
                            <div class="modal__footer">
                                <p>
                                    <a href="javascript:;" data-modal="contact" class="button button--primary button--small">Interesse in dit product?<svg><use xlink:href="/assets/svg/icons.svg#envelope"></use></svg></a></p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div id="overlay" class="modal__overlay"></div>
        </div>

        <div class="modal" id="bbqkar">
            <div class="modal__container">
                <div class="modal__wrapper">
                    <div class="modal__img">
                        <p class="modal__pin">                                    
                            <a  data-pin-do="buttonPin" 
                                href="https://www.pinterest.com/pin/create/button/
                                    ?url=http://www.tech-tree.nl
                                    &media=http://localhost:3000/media/portfolio/treetech-9.jpg
                                    &description=BBQ kar van Tech-Tree.nl" 
                                data-pin-height="28"
                                class="right"></a>
                        </p>
                        <img class="" src="/media/portfolio/treetech-9.jpg">
                    </div>
                    <div class="modal__info">
                        <div class="modal__content"> 
                            <h1 class="modal__title">
                                BBQ kar
                                <a href="javascript:;" class="modal__close"><svg><use xlink:href="/assets/svg/icons.svg#times-circle"></use></svg></a>
                            </h1>
                            <div class="modal__body">
                                <p>Vivamus condimentum accumsan purus, at blandit felis laoreet eu. Nullam nec leo eget nisi scelerisque convallis.</p>
                                <p>Nunc mollis semper diam auctor ultricies. Suspendisse gravida tempus quam, sit amet rhoncus magna vulputate id. Integer sed libero et enim porta bibendum. Duis rhoncus volutpat pellentesque.</p>
                                <p>Morbi efficitur arcu sit amet felis aliquam eleifend. Vivamus ac pretium sem. Proin posuere venenatis lobortis.</p>
                                <p><strong>Prijs op aanvraag</strong></p>
                            </div>
                            <div class="modal__footer">
                                <p>
                                    <a href="javascript:;" data-modal="contact" class="button button--primary button--small">Interesse in dit product?<svg><use xlink:href="/assets/svg/icons.svg#envelope"></use></svg></a></p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div id="overlay" class="modal__overlay"></div>
        </div>

        <div class="modal" id="borrelboom">
            <div class="modal__container">
                <div class="modal__wrapper">
                    <div class="modal__img">
                        <p class="modal__pin">                                    
                            <a  data-pin-do="buttonPin" 
                                href="https://www.pinterest.com/pin/create/button/
                                    ?url=http://www.tech-tree.nl
                                    &media=http://localhost:3000/media/portfolio/treetech-3.jpg
                                    &description=Borrelboom van Tech-Tree.nl" 
                                data-pin-height="28"
                                class="right"></a>
                        </p>
                        <img class="" src="/media/portfolio/treetech-3.jpg">
                    </div>
                    <div class="modal__info">
                        <div class="modal__content"> 
                            <h1 class="modal__title">
                                Borrelboom
                                <a href="javascript:;" class="modal__close"><svg><use xlink:href="/assets/svg/icons.svg#times-circle"></use></svg></a>
                            </h1>
                            <div class="modal__body">
                                <p>Vivamus condimentum accumsan purus, at blandit felis laoreet eu. Nullam nec leo eget nisi scelerisque convallis.</p>
                                <p>Nunc mollis semper diam auctor ultricies. Suspendisse gravida tempus quam, sit amet rhoncus magna vulputate id. Integer sed libero et enim porta bibendum. Duis rhoncus volutpat pellentesque.</p>
                                <p>Morbi efficitur arcu sit amet felis aliquam eleifend. Vivamus ac pretium sem. Proin posuere venenatis lobortis.</p>
                                <p><strong>Prijs op aanvraag</strong></p>
                            </div>
                            <div class="modal__footer">
                                <p>
                                    <a href="javascript:;" data-modal="contact" class="button button--primary button--small">Interesse in dit product?<svg><use xlink:href="/assets/svg/icons.svg#envelope"></use></svg></a></p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div id="overlay" class="modal__overlay"></div>
        </div>

        <div class="modal" id="hangtafel">
            <div class="modal__container">
                <div class="modal__wrapper">
                    <div class="modal__img">
                        <p class="modal__pin">                                    
                            <a  data-pin-do="buttonPin" 
                                href="https://www.pinterest.com/pin/create/button/
                                    ?url=http://www.tech-tree.nl
                                    &media=http://localhost:3000/media/portfolio/treetech-1.jpg
                                    &description=Hangtafel van Tech-Tree.nl" 
                                data-pin-height="28"
                                class="right"></a>
                        </p>
                        <img class="" src="/media/portfolio/treetech-1.jpg">
                    </div>
                    <div class="modal__info">
                        <div class="modal__content"> 
                            <h1 class="modal__title">
                                Hangtafel
                                <a href="javascript:;" class="modal__close"><svg><use xlink:href="/assets/svg/icons.svg#times-circle"></use></svg></a>
                            </h1>
                            <div class="modal__body">
                                <p>Vivamus condimentum accumsan purus, at blandit felis laoreet eu. Nullam nec leo eget nisi scelerisque convallis.</p>
                                <p>Nunc mollis semper diam auctor ultricies. Suspendisse gravida tempus quam, sit amet rhoncus magna vulputate id. Integer sed libero et enim porta bibendum. Duis rhoncus volutpat pellentesque.</p>
                                <p>Morbi efficitur arcu sit amet felis aliquam eleifend. Vivamus ac pretium sem. Proin posuere venenatis lobortis.</p>
                                <p><strong>Prijs op aanvraag</strong></p>
                            </div>
                            <div class="modal__footer">
                                <p>
                                    <a href="javascript:;" data-modal="contact" class="button button--primary button--small">Interesse in dit product?<svg><use xlink:href="/assets/svg/icons.svg#envelope"></use></svg></a></p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div id="overlay" class="modal__overlay"></div>
        </div>

        <div class="modal modal--contact" id="contact">
            <div class="modal__container">
                <div class="modal__wrapper">
                    <div class="modal__info">
                        <div class="modal__content"> 
                            <h1 class="modal__title">
                                Contact
                                <a href="javascript:;" class="modal__close"><svg><use xlink:href="/assets/svg/icons.svg#times-circle"></use></svg></a>
                                <div class="modal__avatar" style="background-image:url(/assets/img/avatar-klein.png);"></div>
                            </h1>
                            <div class="modal__body modal__body--left">
                                <p>Vivamus condimentum accumsan purus, at blandit felis laoreet eu. Nullam nec leo eget nisi scelerisque convallis.</p>
                                <p>Nunc mollis semper diam auctor ultricies. Suspendisse gravida tempus quam, sit amet rhoncus magna vulputate id.</p>
                                <p><strong><svg><use xlink:href="/assets/svg/icons.svg#phone"></use></svg>+31 6 20 40 33 32</strong></p>
                            </div>
                            <div class="modal__body modal__body--right">
                                <form action="" method="POST" id="contactForm" name="contactForm" class="form">
                                    <div class="form__row">
                                        <label for="name">Uw naam:</label>
                                        <input type="text" name="name" id="name" value="Theo Jansen">
                                    </div>
                                    <div class="form__row">
                                        <label for="email">Uw E-mailadres:</label>
                                        <input type="email" name="email" id="email" value="tjanssen@ziggo.nl">
                                    </div>
                                    <div class="form__row">
                                        <label for="message">Uw bericht:</label>
                                        <textarea name="message" rows="6" id="message">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Quia placeat tenetur unde sequi iusto, voluptas.</textarea>
                                    </div>
                                    <div class="form__row">
                                        <button type="submit" class="button button--primary button--small">Verzend</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div id="overlay" class="modal__overlay"></div>
        </div>

        <script src="/assets/js/build/app.min.js"></script>
        <script type="text/javascript" async defer src="//assets.pinterest.com/js/pinit.js"></script>

        <!-- Google Analytics: change UA-XXXXX-X to be your site's ID. -->
        <script>
            (function(b,o,i,l,e,r){b.GoogleAnalyticsObject=l;b[l]||(b[l]=
            function(){(b[l].q=b[l].q||[]).push(arguments)});b[l].l=+new Date;
            e=o.createElement(i);r=o.getElementsByTagName(i)[0];
            e.src='https://www.google-analytics.com/analytics.js';
            r.parentNode.insertBefore(e,r)}(window,document,'script','ga'));
            ga('create','UA-XXXXX-X','auto');ga('send','pageview');
        </script>
    </body>
</html>